...
public class GWTClient implements ComClient
{
	...
	private int port;
	private org.java_websocket.WebSocket wsc; //Websocket to connect to a remote server
	...
	public void connectClient (String ip)
	{
		...
		if (!ip.isEmpty())
		{
			...
			try
			{
				wsc = (org.java_websocket.WebSocket) WebSocket.create(url); //"ws://echo.websocket.org" //For the echo testing server

				//Handler methods override the original methods for the webSocket functionality
				((WebSocket) wsc).setOnOpen(new OpenHandler()
				{
					@Override
				...
				((WebSocket) wsc).setOnMessage(new MessageHandler()
				{
					@Override
				...
				((WebSocket) wsc).setOnError(new ErrorHandler()
				{
					@Override
				...
				((WebSocket) wsc).setOnClose(new CloseHandler()
				{
					@Override
